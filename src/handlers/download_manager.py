...
async def handle_pause(update: Update, context: CallbackContext, db: DatabaseManager, queue_manager):
    """Handle pause button."""
    query = update.callback_query
    ...
    try:
        ...
        await queue_manager.pause_download(download_id, "Paused by user via download manager")
...
async def handle_resume(update: Update, context: CallbackContext, db: DatabaseManager, queue_manager):
    ...
    try:
        download_id = int(query.data.split("_")[-1])
        await queue_manager.resume_download(download_id)
        await show_download_manager(update, context, db)
    except (ValueError, IndexError) as e:
        logger.error(f"Failed to parse download_id from callback data: {query.data}")
        await query.answer("Error: Invalid download ID")
...
async def handle_cancel(update: Update, context: CallbackContext, db: DatabaseManager, queue_manager):
    ...
    try:
        download_id = int(query.data.split("_")[-1])
        await queue_manager.cancel_download(download_id)
        await show_download_manager(update, context, db)
    except (ValueError, IndexError) as e:
        logger.error(f"Failed to parse download_id from callback data: {query.data}")
        await query.answer("Error: Invalid download ID")
